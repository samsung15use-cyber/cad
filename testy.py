...
PRODUCTS = {
    "stars": {
        "name": "Stars",
        "items": {
            "100": {"price": 70, "description": "100 Stars", "amount": 100},
            "500": {"price": 300, "description": "500 Stars -10%", "amount": 500},
            "1000": {"price": 700, "description": "1000 Stars -20%", "amount": 1000},
            "custom": {"price": None, "description": "Своя сумма", "amount": None}
        }
    },
    "premium": {
        "name": "Premium",
    ...
    "ton": {
        "name": "TON",
        "items": {
            "1": {"price": 90, "description": "1 TON", "amount": 1},
            "5": {"price": 300, "description": "5 TON -4%", "amount": 5},
            "10": {"price": 700, "description": "10 TON -8%", "amount": 10},
            "custom": {"price": None, "description": "Своя сумма", "amount": None}
        }
    }
}
...
class Database:
    ...
    def create_order(self, user_id, product_type, product_item, amount, quantity=1, payment_method="balance", recipient_info=None, custom_amount_value=None):
        self.cursor.execute("""
            INSERT INTO orders (user_id, product_type, product_item, amount, quantity, payment_method, created_at, status, recipient_info, custom_amount_value)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_id, product_type, product_item, amount, quantity, payment_method, datetime.now(), "pending", recipient_info, custom_amount_value))
...
def get_product_items_keyboard(product_key):
    ...
    for item_key, item in product["items"].items():
        if item_key == "custom":
            builder.button(text=item["description"], callback_data=f"item_{product_key}_custom")
        else:
...
def get_recipient_choice_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="Купить себе", callback_data="recipient_self")
    builder.button(text="Купить другу", callback_data="recipient_friend")
    builder.button(text="Назад", callback_data="back_to_products")
    builder.adjust(1)
    return builder.as_markup()
...
def get_admin_orders_keyboard(orders):
    ...
    for order in orders[:10]:
        ...
        recipient_info = order[11] if len(order) > 11 and order[11] else ""
        if recipient_info:
            builder.button(text=f"#{order_id} | {username or user_id} → {recipient_info[:15]} | {amount}₽", callback_data=f"admin_order_{order_id}")
        else:
...
async def process_item_selection(callback, state):
    ...
    if item_key == "custom":
        # Пользователь выбрал свою сумму
        await state.update_data(product_key=product_key, item_key=item_key, is_custom=True)
        await state.set_state(OrderStates.entering_custom_amount)
        
        if product_key == "stars":
            await callback.message.edit_text(
                f"Введите количество Stars (от 100 до 10.000):\n\n"
            )
        elif product_key == "ton":
            await callback.message.edit_text(
                f"Введите количество TON (от 1 до 1.000):\n\n"
            )
        await callback.answer()
        return
    ...
    await state.update_data(
        ...
        is_custom=False,
        custom_amount_value=None
    )
    
    await callback.message.edit_text(
        f"Вы выбрали: {item['description']}\n"
        f"Цена: {item['price']}₽\n\n"
        f"Купить для себя или для друга?",
        reply_markup=get_recipient_choice_keyboard()
    )
    await callback.answer()


@dp.message(OrderStates.entering_custom_amount)
async def process_custom_amount(message, state):
    try:
        data = await state.get_data()
        product_key = data.get("product_key")
        
        amount_value = float(message.text.strip())
        
        # Проверки
        if product_key == "stars":
            if amount_value < 100:
                await message.answer("Минимальное количество Stars: 100\nВведите еще раз:")
                return
            if amount_value > 100000:
                await message.answer("Максимальное количество Stars: 100000\nВведите еще раз:")
                return
            price = calculate_price("stars", amount_value)
            description = f"{int(amount_value)} Stars"
        elif product_key == "ton":
            if amount_value < 1:
                await message.answer("Минимальное количество TON: 1\nВведите еще раз:")
                return
            if amount_value > 1000:
                await message.answer("Максимальное количество TON: 1000\nВведите еще раз:")
                return
            price = calculate_price("ton", amount_value)
            description = f"{amount_value} TON"
        else:
        ...
        await state.update_data(
            ...
            is_custom=True,
            custom_amount_value=amount_value
        )
...
async def recipient_self(callback, state):
    data = await state.get_data()
    await state.update_data(recipient_type="self", recipient_info=None)
...
async def recipient_friend(callback, state):
    await state.set_state(OrderStates.entering_friend_info)
    await callback.message.edit_text(
...
@dp.message(OrderStates.entering_friend_info)
async def process_friend_info(message, state):
    friend_info = message.text.strip()
    
    if friend_info.lower() == '/cancel':
        await state.clear()
        await message.answer("Действие отменено", reply_markup=get_main_keyboard(message.from_user.id in ADMIN_IDS))
        return
    
    data = await state.get_data()
    await state.update_data(recipient_type="friend", recipient_info=friend_info)
...
async def confirm_order(callback, state):
    ...
    custom_amount_value = data.get("custom_amount_value")
    ...
    order_id = db.create_order(user_id, product_key, item_key, amount, recipient_info=recipient_info, custom_amount_value=custom_amount_value)
    db.update_balance(user_id, amount, "subtract")
    
    purchase_text = f"Заказ #{order_id} создан!\n\n"
    if recipient_type == "self":
        purchase_text += f"Товар: {description}\n"
    else:
        purchase_text += f"Подарок для: {recipient_info}\n"
        purchase_text += f"Товар: {description}\n"
    purchase_text += f"Сумма: {amount:.2f}₽\n\nОжидает подтверждения"
    
    await callback.message.edit_text(purchase_text)
    
    for admin_id in ADMIN_IDS:
        try:
            admin_text = f"НОВЫЙ ЗАКАЗ #{order_id}!\n\n"
            admin_text += f"Покупатель: {callback.from_user.full_name}\n"
            if recipient_type == "friend":
                admin_text += f"Получатель: {recipient_info}\n"
            admin_text += f"Товар: {description}\n"
            admin_text += f"Сумма: {amount:.2f}₽"
            if custom_amount_value:
                admin_text += f"\nКоличество: {custom_amount_value}"
...
async def show_orders(callback):
    ...
    for order in orders[:10]:
        ...
        status = order[6]
        created_at = order[8]
        recipient_info = order[11] if len(order) > 11 and order[11] else None
        custom_amount = order[12] if len(order) > 12 and order[12] else None
        
        if product_item == "custom" and custom_amount:
            if product_type == "stars":
                product_name = f"{int(custom_amount)} Stars"
            elif product_type == "ton":
                product_name = f"{custom_amount} TON"
            else:
        ...
        if recipient_info:
            text += f"🎁 Подарок для: {recipient_info}\n"
...
async def admin_order_detail(callback):
    ...
    recipient_info = order[11] if len(order) > 11 else None
    custom_amount = order[12] if len(order) > 12 else None
    ...
    text += f"Покупатель: {full_name}\n"
    if recipient_info:
        text += f"Получатель: {recipient_info}\n"
...
async def complete_order(callback):
    ...
    db.cursor.execute("SELECT o.*, u.user_id, u.full_name FROM orders o JOIN users u ON o.user_id = u.user_id WHERE o.order_id = ?", (order_id,))
    order = db.cursor.fetchone()
    ...
    try:
        if recipient_info:
            await bot.send_message(
                ...
                f"Подарок для {recipient_info} отправлен!\n"
